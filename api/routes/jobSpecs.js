...
// try to find this spec in the database.
 SpecModel.find({
    ...(
      ...
      ((SpecModel === AciSpec || SpecModel === AdiSpec || SpecModel === AeiSpec || SpecModel === BiSpec) && {maxFreq: req.body.maxFreq}),
      //ACI j Check
      ((SpecModel === AciSpec) && {j:req.body.j}),
      //ACI/BI/NDSI fftW Check
      ((SpecModel === AciSpec || SpecModel === BiSpec || SpecModel === NdsiSpec) && {fftW:req.body.fftW}),
    ...
    if(spec.length >= 1){
      res.status(200).json({error:"Item already in database",spec:spec}); //if already created then return data object
    }else{
