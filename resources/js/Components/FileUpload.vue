<template>
  <div
    class="flex flex-col grow dark:bg-slate-900 shadow-inner shadow-sky-200 dark:shadow-cyan-500"
  >
    <div class="p-4 mb-2">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="(item, index) in filtered" :key="index">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right">
                  <div>
                    <div v-if="item.size != ''" class="text-sm font-medium text-gray-900">
                      {{ item.size / 1e6 + "MB" }}
                    </div>
                    <div v-else class="text-sm font-medium text-gray-900">undefined</div>
                  </div>
                </td>
                <td class="py-4 whitespace-nowrap text-center text-sm text-neutral-900">
                  <div v-if="item.date != ''" class="text-sm font-medium text-gray-900">
                    {{ new Date(item.lastModified) }}
                  </div>
                  <div v-else class="text-sm font-medium text-gray-900">Today</div>
                </td>
                <td class="py-4 px-6 whitespace-nowrap text-right text-sm font-medium">
                  <div v-if="item.type != ''" class="text-sm font-medium text-gray-900">
                    {{ item.type }}
                  </div>
                  <div v-else class="text-sm font-medium text-gray-900">unknown</div>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div class="p-12 flex flex-row justify-between">
      ...
      >
        ...
        <input class="form-control" type="file" v-on:change="onMetaChange($event)" />
      </form>
    </div>
  </div>
</template>
  ...
  methods: {
    onFileChange(e) {
      ...
      for (const i of Object.keys(file)) {
        if (file[i].name.split(".").pop() == "wav") this.items.push(file[i]);
      }
    },
    onMetaChange(e) {
      // Why can we change metadata. This makes me cry
      if (this.meta && this.meta.length > 0) this.meta.splice(0, this.meta.length);

      const file = e.target.files;
      const fileExtension = file[0].name.split(".").pop();

      if (fileExtension == "txt" || fileExtension == "csv") this.meta.push(file[0]);
    },
    formSubmit(e) {
      e.preventDefault();
      let currentObj = this;
